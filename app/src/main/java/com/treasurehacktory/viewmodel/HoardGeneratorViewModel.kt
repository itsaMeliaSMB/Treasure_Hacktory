...
import com.treasurehacktory.ui.GenEditTextTag
...
class HoardGeneratorViewModel(private val repository: HMRepository): ViewModel() {
    ...
    fun updateLairEntry(position: Int, newValue : Int) {
        if (position in lairList.indices) {
            lairList[position] = lairList[position].first to newValue.coerceIn(
                MINIMUM_LETTER_QTY,
                MAXIMUM_LETTER_QTY
            )
    ...
    fun updateSmallEntry(position: Int, newValue : Int) {
        if (position in smallList.indices) {
            smallList[position] = smallList[position].first to newValue.coerceIn(
                MINIMUM_LETTER_QTY,
                MAXIMUM_LETTER_QTY
            )
